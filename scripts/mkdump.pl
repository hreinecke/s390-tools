...
sub setup_zfcpdump
{
	...
	my $part_num = get_partition_num($device);
	if ($part_num == 0 or $OPT_FORCE) {
		print("Re-partitioning disk $device and creating ext2 file system.\n");
		run_cmd("$PARTED -s -- $device  mklabel msdos  mkpartfs primary ext2 0 -1");
	} else {
		if ($part_num > 1) {
			exit_with("$part_num partitions detected, cannot use disk $device.", 12);
		} else {
			# allow zFCP disks with one partition and a compatible file systems
			my ($fstype) = `$BLKID ${device}1` =~ /TYPE=\"(\w+)\"/;
			if ($fstype =~ /ext[23]/ == 0)  {
				exit_with("File system type for ${device}1 has to be ext2/3.", 12);
			}
		}
	}
	...
	run_cmd("mkdir -p $MNTPOINT");
	run_cmd("mount ${device}1 $MNTPOINT");
        
	unless ( -d "$MNTPOINT/$ZFCP_DUMP_DIR") {
		run_cmd("mkdir -p $MNTPOINT/$ZFCP_DUMP_DIR");
	}

	print("Creating dump record.\n");
	run_cmd("${ZIPL} -V -D ${device}1 -t ${MNTPOINT} -P \"dump_dir=$ZFCP_DUMP_DIR dump_compress=gzip\"");
