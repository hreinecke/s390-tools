...
/*
 * Read memory for virtual memory chunk
 */
static void mem_chunk_virt_read_fn(struct dfi_mem_chunk *mem_chunk, u64 off,
				   void *buf, u64 cnt)
{
	u64 *start = mem_chunk->data;

	mem_read(&l.mem_phys, *start + off, buf, cnt);
}

/*
 * Check if memory chunk is virtual
 */
static int mem_chunk_is_virt(struct dfi_mem_chunk *mem_chunk)
{
	return mem_chunk->read_fn == mem_chunk_virt_read_fn;
}
...
{
	if (mem_chunk_is_virt(mem_chunk))
 ...
 */
static void mem_chunk_virt_add(u64 start, u64 size, u64 start_p)
{
	u64 *data = zg_alloc(sizeof(*data));

	*data = start_p;
	mem_chunk_create(&l.mem_virt, start, size, data,
			 mem_chunk_virt_read_fn, zg_free);
}
...
static void mem_unmap(u64 start, u64 size)
{
	...
	util_list_iterate_safe(&l.mem_virt.chunk_list, mem_chunk, tmp) {
		...
		if (mem_chunk->end > end) {
			...
			mem_chunk_virt_add(addr_virt, size_virt, addr_phys);
		}
		/*
...
static void mem_map(u64 start, u64 size, u64 start_phys)
{
	...
	mem_chunk_virt_add(start, size, start_phys);
	mem_update(&l.mem_virt);
}
...
static void kdump_init(void)
{
	...
	mem_read(&l.mem_phys, 0x10418, &base, sizeof(base));
	mem_read(&l.mem_phys, 0x10420, &size, sizeof(size));
	if (base == 0 || size == 0)
		return;
