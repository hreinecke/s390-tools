...
int
VMDump::seekMem(uint64_t offset)
{
	if(offset != 0) {
		return -1;
	}
...
RegisterContent32
VMDump32::getRegisterContent(void)
{
...
RegisterContent64
VMDump64::getRegisterContent(void)
{
...
RegisterContent64
VMDump64Big::getRegisterContent(void)
{
	RegisterContent64 rc;
	RegisterSet64     rs;
	int cpu;
	...
	for(cpu = 0; cpu < fir64Record.online_cpus; cpu++) {
		...
		memcpy(&rs.acrs, &fir64OtherRecords[cpu].acrs, sizeof(rs.acrs));
		memcpy(&rs.psw,  &fir64OtherRecords[cpu].psw, sizeof(rs.psw));
		memcpy(&rs.prefix, &fir64OtherRecords[cpu].prefix, sizeof(rs.prefix));
		memcpy(&rs.fprs, &fir64OtherRecords[cpu].fprs, sizeof(rs.fprs));
		memcpy(&rs.cpuTimer, &fir64OtherRecords[cpu].cpu_timer, sizeof(rs.cpuTimer));
		memcpy(&rs.clkCmp, &fir64OtherRecords[cpu].clock_cmp, sizeof(rs.clkCmp));
		memcpy(&rs.fpCr, &fir64OtherRecords[cpu].fp_cntrl_reg, sizeof(rs.fpCr));
		rc.addRegisterSet(rs);
	}
	return rc;
}
